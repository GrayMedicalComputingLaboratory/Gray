...
def roc_auc(targets: Sequence[Any], probabilities: Sequence[float] | Sequence[Sequence[float]] | np.ndarray, labels: Sequence[Any] | None = None) -> float | None:
    """Return ROC-AUC, or ``None`` when the observed targets make it undefined."""
    ...
    try:
        if len(class_labels) == 2:
            ...
            return float(roc_auc_score(y_true == class_labels[1], positive))
        if score_array.ndim != 2 or score_array.shape[1] != len(class_labels):
            raise ValueError("multiclass probabilities must have shape [N, number_of_labels]")
        return float(roc_auc_score(y_true, score_array, labels=class_labels, multi_class="ovr", average="macro"))
    except ValueError:
        return None
